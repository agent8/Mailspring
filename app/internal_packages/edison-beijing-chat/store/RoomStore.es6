...
class RoomStore extends MailspringStore {
  ...
  refreshRoomsFromXmpp = async jid => {
    ...
    xmpp.getRoomList(ver, jid).then(rooms => this.saveRooms(rooms))
  }
  ...
  onMembersChange = async payload => {
    ...
    if (
      ...
      ConversationStore.selectedConversation.jid === conversationJid
    ) {
  ...
  onPadMemberChange = async payload => {
    ...
    const savePadMessage = async body => {
      ...
      console.log(' savePadMessage: msg: ', msg)
      await MessageStore.saveProcessGroupMessage(msg)
      this.trigger()
