...
export default class MessageImagePopup extends Component {
  ...
  getFileName(msgBody) {
    return (
      (msgBody.localFile && path.basename(msgBody.localFile)) ||
      msgBody.mediaObjectId.replace(/\.encrypted$/, '')
    );
  }
  ...
  getAllImages(props) {
    ...
    for (let msg of groupedMessages) {
      ...
      if (
        ...
        msgBody.type === FILE_TYPE.STICKER
      ) {
        imgMsgs.push(msg);
  ...
  render() {
    const { referenceTime, getContactInfoByJid, getContactAvatar } = this.props;
          ...
          >
            <div className="message-image-popup-toolbar">
              ...
              <div className="inline-block">
                <div>{fileName}</div>
                <span>
                  {this.getContactNameByJid(msg.sender)} &nbsp; {timeDescriptor(msg.sentTime, true)}
                </span>
              </div>
