...
export default class OnlineStatus extends Component {
  ...
  _onDataChanged = async () => {
    ...
    if (conversation) {
      ...
      console.log(
        'on chat_online change: curJid, OnlineUserStore.onlineAccounts: ',
        curJid,
        OnlineUserStore.onlineAccounts
      );
    ...
    log(
      'connect',
      `OnlineStatus._onDataChanged, chat_online: ${chat_online}, isAuthenticating: ${isAuthenticating}: onlineAccounts: ${str}, authingAccounts: ${str2}`
    );
    this.setState(
      {
        chat_online,
  ...
  render() {
    ...
    return (
      <div className="network-offline">
        ...
        {this.state.online ? (
          <span className="reconnect" onClick={this.reconnect}>
            Reconnect
          </span>
        ) : null}
      </div>
