...
export const register = (email, pwd, name, type, provider, setting) => {
  ...
  let ssl = setting.imap_allow_insecure_ssl ? true : setting.imap_security === 'SSL / TLS'
...
export async function refreshChatAccountTokens (cb) {
  ...
  for (let acc of accounts) {
    ...
    let { err, res } = await register(
      acc.emailAddress,
      acc.settings.imap_password || acc.settings.refresh_token,
      acc.name,
      type,
      acc.provider,
      acc.settings
