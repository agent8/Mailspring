...
export default async function registerLoginChat () {
  ...
  await ConversationStore.createInitialPrivateConversationsFromAllContacts()
}
...
export async function registerLoginEmailAccountForChat (account) {
  ...
  if (!chatAccount.password || leftTime < 2 * 24 * 3600) {
    ...
    console.log(
      'registerLoginChat account.settings.refresh_token, account.settings.imap_password, account: ',
      account.settings.refresh_token,
    ...
    try {
      const result = await register(
        ...
        account.settings.refresh_token || account.settings.imap_password,
        account.name,
        type,
        account.provider,
        account.settings
