...
class AccountSidebar extends React.Component {
  ...
  _onStoreChange = () => {
    console.log(' on SidebarStore Change:')
    console.log(' SidebarStore.standardSection():', SidebarStore.standardSection())
    if (this._mounted) {
  ...
  _getStateFromStores = () => {
    const section = SidebarStore.standardSection()
    if (window.teamEditActivated) {
      const items = window.getTeamEditSideBarItems()
      section.items.splice(6, 0, ...items)
      console.log(' teamEditActivated section.items: ', section.items)
    }
    return {
  ...
  render () {
    ...
    console.log(' Sidebar.render:', standardSection)
