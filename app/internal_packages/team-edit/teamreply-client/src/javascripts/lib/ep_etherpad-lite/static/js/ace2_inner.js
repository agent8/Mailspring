require.define({
  'ep_etherpad-lite/static/js/ace2_inner.js': function (require, exports, module) {
    ...
    function Ace2Inner () {
      ...
      var DEBUG = true //yazz //$$ build script replaces the string "var DEBUG=true;//$$" with "var DEBUG=false;"
      // changed to false
      var isSetUp = false
      ...
      var FORMATTING_STYLES = ['bold', 'italic', 'underline', 'strikethrough'] //yazz，富文本
      ...
      var sideDiv = outerWin.document.getElementById('sidediv')
      ...
      if (!window.console) {
        ...
        console = {}
        for (var i = 0; i < names.length; ++i) console[names[i]] = noop
      ...
      function setAuthorStyle (author, info) {
        ...
        if (
          _.any(authorStyleSet, function (it) {
            return it
          })
        ) {
          return
        ...
        if (!info) {
          ...
          if (info.bgcolor) {
            ...
            if (colorutils.luminosity(colorutils.css2triple(bgcolor)) < 0.55) {
              anchorStyle.color = colorutils.triple2css(
                colorutils.complementary(colorutils.css2triple(bgcolor))
              )
            } else {
      ...
      function getAuthorClassName (author) {
        return (
          'author-' +
          author.replace(/[^a-y0-9]/g, function (c) {
            if (c == '.') return '-'
      ...
      function setOnKeyPress (handler) {
        // console.log('yazz.setOnKeyPress',handler);
        outsideKeyPress = handler
      }

      function setOnKeyDown (handler) {
        // console.log('yazz.setOnKeyDown',handler);
        outsideKeyDown = handler
      }

      function setNotifyDirty (handler) {
        // console.log('yazz.setNotifyDirty',handler);
        ...
        clearauthorship: function (prompt) {
          if (!(rep.selStart && rep.selEnd) || isCaret()) {
            if (prompt) {
              ...
              performDocumentApplyAttributesToCharRange(0, rep.alltext.length, [['author', '']])
            }
          } else {
      ...
      editorInfo.ace_prepareUserChangeset = function () {
        // console.log('yazz.changesetTracker.prepareUserChangeset');
      ...
      var idleWorkTimer = makeIdleAction(function () {
        ...
        inCallStackIfNecessary('idleWorkTimer', function () {
          var isTimeUp = newTimeLimit(2500) //yazz，250--》2500
          // console.log('yazz.inCallStackIfNecessary',now());
      ...
      function getSpansForLine (lineEntry, textAndClassFunc, lineEntryOffsetHint) {
        ...
        if (text.length === 0) {
          ...
          var func = linestylefilter.getLineStyleFilter(0, '', textAndClassFunc, rep.apool)
          func('', '')
        } else {
      ...
      function incorporateUserChanges (isTimeUp) {
        ...
        while (i < dirtyRanges.length) {
          ...
          var lastDirtyNode =
            (b == rep.lines.length() && root.lastChild) ||
      ...
      function handleReturnIndentation () {
        ...
        if (isCaret() && caretColumn() === 0 && caretLine() > 0) {
          ...
          var cs = Changeset.builder(rep.lines.totalWidth())
      ...
      function performDocumentApplyChangeset (changes, insertsAfterSelection) {
        ...
        if (rep.selStart && rep.selEnd) {
          var selStartChar = rep.lines.offsetOfIndex(rep.selStart[0]) + rep.selStart[1]
          var selEndChar = rep.lines.offsetOfIndex(rep.selEnd[0]) + rep.selEnd[1]
          var result = Changeset.characterRangeFollow(
            changes,
            selStartChar,
            selEndChar,
        ...
        function domAndRepSplice (startLine, deleteCount, newLineStrings, isTimeUp) {
          ...
          insertDomLines(
            nodeToAddAfter,
            _.map(lineEntries, function (entry) {
      ...
      function doRepApplyChangeset (changes, insertsAfterSelection) {
        ...
        if (Changeset.oldLen(changes) != rep.alltext.length)
          throw new Error(
            'doRepApplyChangeset length mismatch: ' +
              Changeset.oldLen(changes) +
              '/' +
              rep.alltext.length
          )
        ...
        ;(function doRecordUndoInformation (changes) {
          ...
          if (editEvent.eventType == 'nonundoable') {
            ...
            var inverseChangeset = Changeset.inverse(
              changes,
              {
                get: function (i) {
                  return rep.lines.atIndex(i).text + '\n'
      ...
      */
      ...
      function doIncorpLineSplice (startLine, deleteCount, newLineEntries, lineAttribs, hints) {
        ...
        if (hints && hints.selEnd) {
          selEndHintChar =
            rep.lines.offsetOfIndex(hints.selEnd[0]) + hints.selEnd[1] - oldRegionStart
        }
      ...
      function repSelectionChange (selectStart, selectEnd, focusAtStart) {
        ...
        if (
          ...
          selectFormattingButtonIfLineHasStyleApplied(rep) //yazz，更新button
          ...
          if (!docTextChanged) {
            var isScrollableEvent =
              !isPadLoading(currentCallStack.type) && isScrollableEditEvent(currentCallStack.type)
      ...
      function selectFormattingButtonIfLineHasStyleApplied (rep) {
        _.each(FORMATTING_STYLES, function (style) {
          var hasStyleOnRepSelection = documentAttributeManager.hasAttributeOnSelectionOrCaretPosition(
            style
          )
      ...
      function getViewPortTopBottom () {
        ...
        var viewportExtraSpacesAndPosition =
      ...
      function hideEditBarDropdowns () {
        if (window.parent.parent.padeditbar) {
          // required in case its in an iframe should probably use parent..  See Issue 327 https://github.com/ether/etherpad-lite/issues/327
          ...
          if (!handled) {
            if (isCaret()) {
              ...
              if (caretColumn() <= lineEntry.lineMarker) {
                ...
                var prevLineBlank =
                  prevLineEntry && prevLineEntry.text.length == prevLineEntry.lineMarker
      ...
      function handleKeyEvent (evt) {
        ...
        var caretOffsetTop =
          myselection.focusNode.parentNode.offsetTop | myselection.focusNode.offsetTop // get the carets selection offset in px IE 214
        ...
        var isTypeForSpecialKey =
          browser.msie || browser.safari || browser.chrome || browser.firefox
            ? type == 'keydown'
            : type == 'keypress'
        var isTypeForCmdKey =
          browser.msie || browser.safari || browser.chrome || browser.firefox
        ...
        inCallStackIfNecessary('handleKeyEvent', function () {
          ...
          if (!stopped) {
            ...
            if (
              ...
              // console.log('yazz.alineAttrs',alineAttrs);
              var author = null
              if (alineAttrs) {
                var authors = []
                var authorNames = []
            ...
            if (
              ...
              /* Do a saved revision on ctrl S */ evt.preventDefault()
            ...
            if (evt.which == 36 && evt.ctrlKey == true && padShortcutEnabled.ctrlHome) {
              scroll.setScrollY(0)
      ...
      function initLineNumbers () {
        lineNumbersShown = 1
        sideDiv.innerHTML =
          '<table border="0" cellpadding="0" cellspacing="0" align="right"><tr><td id="sidedivinner" class="sidedivinner"><div>1</div></td></tr></table>'
      ...
      editorInfo.ace_performDocumentApplyAttributesToRange = function () {
        return documentAttributeManager.setAttributesOnRange.apply(
      ...
      this.init = function () {
        $(document).ready(function () {
          ...
          inCallStack('setup', function () {
            ...
            rep.alines = Changeset.splitAttributionLines(Changeset.makeAttribution('\n'), '\n')
  ...
  'ep_etherpad-lite/static/js/Changeset.js': function (require, exports, module) {
    ...
    exports.toBaseTen = function (cs) {
      ...
      return (
        beforeDollar.replace(/[0-9a-z]+/g, function (s) {
        ...
        if (L) {
          ...
          if (isCurLineInSplice()) {
            ...
            if (curCol == 0) {
              ...
              curSplice[sline] =
                curSplice[sline].substring(0, curCol) + lines_get(curSplice[0] + curSplice[1])
              curSplice[1] += 1
            }
          } else {
      ...
      function remove (N, L) {
        ...
        if (N) {
          if (L) {
            ...
            curSplice[sline] =
              curSplice[sline].substring(0, curCol) + curSplice[sline].substring(curCol + N)
            //debugPrint("remove");
          }
        }
        return removed
    ...
    exports.moveOpsToNewPool = function (cs, oldPool, newPool) {
      ...
      return (
        upToDollar.replace(/\*([0-9a-z]+)/g, function (_, a) {
          ...
          if (!pair)
            exports.error(
              "Can't copy unknown attrib (reference attrib string to non-existant pool entry). Inconsistent attrib state!"
            )
    ...
    function makeContentCollector (collectStyles, abrowser, apool, domInterface, className2Author) {
      ...
      cc.collectContent = function (node, state) {
        ...
        if (dom.isNodeText(node)) {
          ...
          var txt =
            typeof txtFromHook == 'object' && txtFromHook.length == 0
              ? dom.nodeValue(node)
              : txtFromHook[0]
          ...
          if (tname == 'br') {
            ...
            var startNewLine =
              typeof induceLineBreak == 'object' && induceLineBreak.length == 0
                ? true
                : induceLineBreak[0]
            if (startNewLine) {
              cc.startNewLine(state)
    ...
    function makeChangesetTracker (scheduler, apool, aceCallbacksProvider) {
      ...
      return (self = {
        ...
        composeUserChangeset: function (c) {
          ...
          // console.log('yazz.composeUserChangeset',c);
        ...
        applyChangesToBase: function (c, optAuthor, apoolJsonObj) {
          ...
          aceCallbacksProvider.withCallbacks('applyChangesToBase', function (callbacks) {
            ...
            var postChange = Changeset.follow(
              oldUserChangeset,
              c2,
              !preferInsertingAfterUserChanges,
              apool
          ...
          if (submittedChangeset) {
            ...
            if (apool.numToAttrib) {
              for (var attr in apool.numToAttrib) {
                if (
                  apool.numToAttrib[attr][0] == 'author' &&
  ...
  'ep_etherpad-lite/static/js/linestylefilter.js': function (require, exports, module) {
    ...
    linestylefilter.getLineStyleFilter = function (lineLength, aline, textAndClassFunc, apool) {
      ...
      hooks.aCallAll('aceAttribClasses', linestylefilter.ATTRIB_CLASSES, function (
        err,
        ATTRIB_CLASSES
      ) {
      ...
      var authorColorFunc = (function () {
        ...
        return function (txt, cls) {
          var disableAuthColorForThisLine = hooks.callAll(
          ...
          var disableAuthors =
            disableAuthColorForThisLine == null || disableAuthColorForThisLine.length == 0
              ? false
    ...
    linestylefilter.REGEX_URLCHAR = new RegExp(
      '(' +
        /[-:@a-zA-Z0-9_.,~%+\/\\?=&#!;()$]/.source +
        '|' +
        linestylefilter.REGEX_WORDCHAR.source +
        ')'
    )
    linestylefilter.REGEX_URL = new RegExp(
      /(?:(?:https?|s?ftp|ftps|file|nfs):\/\/|(about|geo|mailto|tel):|www\.)/.source +
        linestylefilter.REGEX_URLCHAR.source +
        '*(?![:.,;])' +
        linestylefilter.REGEX_URLCHAR.source,
      'g'
    )
    linestylefilter.getURLFilter = linestylefilter.getRegexpFilter(linestylefilter.REGEX_URL, 'url')
    ...
    linestylefilter.getFilterStack = function (lineText, textAndClassFunc, abrowser) {
      ...
      if (abrowser !== undefined && abrowser.msie) {
        ...
        func = linestylefilter.getAtSignSplitterFilter(lineText, func)
      }
      return func
    }
    ...
    domline.createDomLine = function (nonEmpty, doesWrap, optBrowser, optDocument) {
      ...
      result.appendSpan = function (txt, cls) {
        ...
        if (cls.indexOf(lineAttributeMarker) >= 0) {
              ...
              if (listType.indexOf('number') < 0) {
                ...
                if (start) {
                  ...
                  preHtml +=
                    ...
                    start[1] +
                    ' class="list-' +
                    Security.escapeHTMLAttribute(listType) +
                    '"><li>'
                } else {
          ...
          console.log('appendSpan: txt, cls:', txt, cls)
          _.map(
    ...
    AttributeManager.prototype = _(AttributeManager.prototype).extend({
      ...
      */
      ...
      _findRowRange: function (row, start, end) {
        ...
        if (row === start[0]) {
          // are we on the first row of range?
          startCol = start[1]
        } else {
        ...
        if (row === end[0]) {
          // are we on the last row of range?
      ...
      */
      lineHasMarker: function (lineNum) {
        ...
        return (
          _.find(lineAttributes, function (attribute) {
      ...
      */
      ...
      removeAttributeOnLine: function (lineNum, attributeName, attributeValue) {
        ...
        var countAttribsWithMarker = _.chain(attribs)
          .filter(function (a) {
            return !!a[1]
          })
          .map(function (a) {
