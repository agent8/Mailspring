...
import MessagesSendBar from '../../../edison-beijing-chat/components/chat/messages/MessagesSendBar';
import Messages from '../../../edison-beijing-chat/components/chat/messages/Messages';
import Divider from '../../../edison-beijing-chat/components/common/Divider';
import { downloadFile, uploadFile } from '../../../edison-beijing-chat/utils/awss3';
import { postAsync } from '../../../edison-beijing-chat/utils/httpex'
import registerLoginChat from '../../../edison-beijing-chat/utils/register-login-chat';
...
import { xmpplogin } from '../../../edison-beijing-chat/utils/restjs';
import fs from 'fs';
import https from 'https';
import http from 'http';
import { MESSAGE_STATUS_TRANSFER_FAILED } from '../../../edison-beijing-chat/model/Message';
import { sendFileMessage } from '../../../edison-beijing-chat/utils/message';
import { getToken } from '../../../edison-beijing-chat/utils/appmgt';
import { alert } from '../../../edison-beijing-chat/utils/electron-utils';
import { log } from '../../../edison-beijing-chat/utils/log';

const { exec } = require('child_process');
import { NEW_CONVERSATION } from '../../../edison-beijing-chat/utils/constant';
...
export default class MessagesPanel extends Component {
  ...
  _onDataChanged = async changedDataName => {
    if (changedDataName === 'conversation') {
      ...
      console.log( ' PadMessagePanel._onDataChanged, selectedConversation: ', selectedConversation)
      if (
  ...
  componentDidMount = async () => {
    ...
    const contacts = await ContactStore.getContacts()
    this.unsub = PadStore.listen(this.updatePad)
    const {padInfo} = this.props
    const {padId, userId, token} = padInfo
    let pad = await PadStore.getPadByPadId(padId)
    console.log( 'PadMessagePanel.componentDidMount, PadStore, padInfo, pad:', PadStore, padInfo, pad)
    let res
    if (!pad) {
      const apiPath = window.teamPadConfig.teamEditAPIUrl + 'getPad'
      const options = { userId, token, padID:padId }
      res = await postAsync(apiPath, options, {
        headers: {
      ...
      if (typeof res === 'string') {
        try { res = JSON.parse(res)
          ...
          console.error('error in parsing getPad response:', res, e)
        }
      }
      console.log( 'PadMessagePanel.componentDidMount res:', res)
      if (!res) {
        this.setState({
  ...
  updatePad = async () => {
    ...
    const {chatRoomId} = pad
    const convJid = `${chatRoomId}@muc.im.edison.tech`
    await ConversationStore.setSelectedConversation(convJid)
  ...
  render() {
    console.log( 'PadMessagePanel.render: state:', this.state)
    const { selectedConversation } = this.state;
    const currentUserId =
