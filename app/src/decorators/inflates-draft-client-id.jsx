...
function InflatesDraftClientId (ComposedComponent) {
  return class extends React.Component {
    ...
    componentDidMount () {
      ...
      if (
        ...
        if (
          ...
          AppEnv.reportError(new Error('Draft editing session should not have sending/failing state drafts'), {
    ...
    UNSAFE_componentWillReceiveProps (newProps) {
      if (newProps.headerMessageId !== this.props.headerMessageId || newProps.messageId !== this.props.messageId) {
    ...
    _prepareForDraft (headerMessageId, messageId) {
      ...
      DraftStore.sessionForClientId(headerMessageId).then(session => {
        const shouldSetState = () => {
          ...
          return this._mounted && (session.headerMessageId === this.props.headerMessageId || sameDraftWithNewID)
        }
        if (!shouldSetState()) {
          // console.log('-------------------inflate-draft-cilent-id--------------- ');
          // console.log('did not update state')
