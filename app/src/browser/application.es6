...
export default class Application extends EventEmitter {
  ...
  handleEvents () {
    ...
    ipcMain.on('send-later-manager', (event, action, headerMessageId, delay, actionKey, threadId) => {
      ...
      if (action === 'send-later') {
        ...
        clearTimeout(timer)
        delete this._draftsSendLater[headerMessageId]
        mainWindow.browserWindow.webContents.send('action-send-cancelled', headerMessageId, actionKey)
        if (threadId) {
    ...
    ipcMain.on('draft-got-new-id', (event, options) => {
      ...
      const mainWindow = this.windowManager.get(WindowManager.MAIN_WINDOW)
      if (mainWindow && mainWindow.browserWindow.webContents) {
        mainWindow.browserWindow.webContents.send(`${additionalChannelParam}draft-got-new-id`, options)
      }
      if (options.threadId) {
        const threadWindow = this.windowManager.get(`thread-${options.threadId}`)
        if (threadWindow && threadWindow.browserWindow.webContents) {
          threadWindow.browserWindow.webContents.send(`${additionalChannelParam}draft-got-new-id`, options)
        }
      }
      if (options.oldHeaderMessageId && options.newHeaderMessageId) {
        ...
        if (composerWindow && composerWindow.browserWindow.webContents) {
          composerWindow.browserWindow.webContents.send(`${additionalChannelParam}draft-got-new-id`, options)
        } else {
    ...
    ipcMain.on('new-window', (event, options) => {
      ...
      if (win) {
        ...
        if (options.threadId) {
          const threadWindow = this.windowManager.get(`thread-${options.threadId}`)
          if (threadWindow && threadWindow.browserWindow.webContents) {
            threadWindow.browserWindow.webContents.send(`${additionalChannelParam}new-window`, options)
          }
        }
      }
    })
    ...
    ipcMain.on('close-window', (event, options) => {
      ...
      if (options.threadId) {
        const threadWindow = this.windowManager.get(`thread-${options.threadId}`)
        if (threadWindow && threadWindow.browserWindow.webContents) {
          threadWindow.browserWindow.webContents.send(`${additionalChannelParam}close-window`, options)
        }
      }
    })
  ...
  runSpecs (specWindowOptionsArg) {
    ...
    try {
      bootstrapScript = require.resolve(path.resolve(this.resourcePath, 'spec', 'spec-runner', 'spec-bootstrap'))
    } catch (error) {
      bootstrapScript = require.resolve(path.resolve(__dirname, '..', '..', 'spec', 'spec-runner', 'spec-bootstrap'))
    }
