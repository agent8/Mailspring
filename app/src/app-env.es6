...
export default class AppEnvConstructor {
  ...
  setupErrorLogger () {
    ...
    window.addEventListener('unhandledrejection', e => {
      ...
      const error = new Error(
        `Unrecognized event shape in window.unhandledrejection handler. Event keys: ${Object.keys(e)}`
      )
      this._onUnhandledRejection(error, sourceMapCache)
    })
  ...
  */
  ...
  setPosition (x, y) {
    return ipcRenderer.send('call-window-method', 'setPosition', ensureInteger(x, 0), ensureInteger(y, 0))
  }
  ...
  populateHotWindow (event, loadSettings) {
    ...
    this.emitter.emit('window-props-received', loadSettings.windowProps != null ? loadSettings.windowProps : {})
  ...
  */
  ...
  showOpenDialog (options, callback) {
    return remote.dialog.showOpenDialog(this.getCurrentWindow(), options).then(({ canceled, filePaths }) => {
      if (canceled) {
        callback(null)
      } else {
        callback(filePaths)
  ...
  showSaveDialog (options, callback) {
    ...
    if (downloadPath) {
      ...
      if (fs.existsSync(path.join(downloadPath, fileName))) {
        ...
        fileName = `${path.basename(fileName, extname)}_${moment().format('YYYY-MM-DD_HH:mm:ss')}${extname}`
      }
      ...
      return remote.dialog.showSaveDialog(this.getCurrentWindow(), options).then(({ canceled, filePath }) => {
        if (canceled) {
          callback(null)
        } else {
          callback(filePath)
