...
class KeyManager {
  ...
  async extractChatAccountSecrets (account) {
    try {
      ...
      const keys = await this._getKeyHash(this.CHAT_PREFIX)
      keys[`${email}-password`] = account.password
      keys[`${email}-accessToken`] = account.accessToken
  ...
  async insertChatAccountSecrets (account) {
    ...
    const keys = await this._getKeyHash(this.CHAT_PREFIX)
    let email = account.email && account.email.toLowerCase()
    if (keys[`${email}-password`]) {
      next.password = keys[`${email}-password`]
    }
    if (keys[`${account.email}-accessToken`]) {
      next.accessToken = keys[`${email}-accessToken`]
    }
    return next
  }
