...
export default class MailsyncProcess extends EventEmitter {
        ...
        if (lastJSON) {
          if (lastJSON.error) {
            ...
            this.emit(
              'deltas',
              [outBuffer],
              this.account ? this.account.id || this.account.pid : ''
            );
          }
        }
      }
  ...
  async migrate() {
    try {
      ...
      let migrateReturn;
      try {
        migrateReturn = buffer.toString().trim();
        ret = JSON.parse(migrateReturn);
      } catch (err) {
        console.log(`migrate() json parse error ${migrateReturn}`);
      }
